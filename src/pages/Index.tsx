...
type Tab = "catalog" | "search" | "compare" | "reviews";
...
const REVIEWS = [
  {
    ...
    rating: 5,
    date: "12 апреля 2026",
    text: "Брал для домашней сборки — просто огонь за такие деньги. Температуры адекватные, в играх никаких тормозов. Очень доволен покупкой, всем советую!",
  },
  {
    id: 2,
    author: "Марина В.",
    avatar: "МВ",
    product: "Kingston Fury 16GB DDR4",
    category: "Оперативная память",
    rating: 4,
    date: "9 апреля 2026",
    text: "Хорошая оперативка за небольшие деньги. Работает стабильно, без глюков. Единственное — пришлось вручную выставить профиль в BIOS, но это мелочи.",
  },
  {
    id: 3,
    author: "Дмитрий П.",
    ...
    rating: 5,
    date: "7 апреля 2026",
    text: "Для игр в Full HD — идеальный вариант. Тянет всё что мне нужно на высоких настройках. Тихая, не греется, потребляет мало. Очень доволен!",
  },
  {
    id: 4,
    author: "Сергей Н.",
    ...
    rating: 5,
    date: "5 апреля 2026",
    text: "Мощнейшая карта, 4K без проблем. DLSS 3 меняет игру полностью — FPS вырос в полтора раза. Дорого, но оно того стоит, если берёте надолго.",
  },
  {
    id: 5,
    author: "Ольга Р.",
    ...
    rating: 4,
    date: "3 апреля 2026",
    text: "Быстрый и недорогой SSD. Windows загружается за несколько секунд. Скоростью доволна, по надёжности пока рано судить, но всё работает отлично.",
  },
  {
    id: 6,
    author: "Игорь Ф.",
    avatar: "ИФ",
    product: "Deepcool PQ650M",
    category: "Блоки питания",
    rating: 5,
    date: "1 апреля 2026",
    text: "Отличный блок питания за свои деньги. Тихий, полумодульный — кабели не мешают. 80+ Gold — потребление в норме. Рекомендую для бюджетных сборок.",
  },
  {
    id: 7,
    author: "Антон М.",
    ...
    rating: 4,
    date: "28 марта 2026",
    text: "16 ГБ видеопамяти — огромный плюс для будущих игр. В рейтрейсинге слабее RTX, но в растеризации держится хорошо. Соотношение цена/качество на высоте.",
  },
  {
    id: 8,
    author: "Наталья С.",
    avatar: "НС",
    product: "Crucial 16GB DDR4",
    category: "Оперативная память",
    rating: 3,
    date: "25 марта 2026",
    text: "Работает нормально, нареканий нет. Но тайминги CL22 — не лучший показатель. Если хотите что-то побыстрее, доплатите немного за Kingston Fury.",
  },
  {
    id: 9,
    author: "Павел Д.",
    ...
    rating: 5,
    date: "14 апреля 2026",
    text: "Взял для сборки на 1080p — просто идеальный выбор. Все современные игры идут на высоких настройках без просадок. Карта тихая, греется умеренно. За эти деньги лучшего варианта нет.",
  },
  {
    id: 10,
    author: "Виктор Л.",
    ...
    rating: 4,
    date: "11 апреля 2026",
    text: "Хорошая бюджетная карта для 1080p. Драйверы AMD стали намного лучше чем раньше. Немного шумит под нагрузкой, но в целом очень доволен соотношением цены и производительности.",
  },
  {
    id: 11,
    author: "Екатерина Ж.",
    ...
    rating: 5,
    date: "8 апреля 2026",
    text: "Брала для работы с 3D и видеомонтажа — справляется на отлично. CUDA-ядра делают рендер в разы быстрее. Плюс в играх полный восторг: 1440p на максималках без компромиссов.",
  },
];
      ...
      <header className="border-b border-border bg-card sticky top-0 z-40">
        <div className="max-w-screen-xl mx-auto px-6 h-14 flex items-center justify-between gap-4">
          ...
          <nav className="flex items-center gap-1">
            {(["catalog", "search", "compare", "reviews"] as Tab[]).map((t) => {
              const labels = { catalog: "Каталог", search: "Поиск", compare: "Сравнение", reviews: "Отзывы" };
              const icons = { catalog: "LayoutGrid", search: "Search", compare: "GitCompare", reviews: "MessageSquare" };
              return (
                <button
                  key={t}
                  onClick={() => setTab(t)}
...
function ReviewsSection() {
  ...
  const filtered = filter === "Все" ? REVIEWS : REVIEWS.filter((r) => r.category === filter);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-base font-semibold">
          Отзывы покупателей
          <span className="ml-2 text-sm font-normal text-muted-foreground">{REVIEWS.length} отзывов</span>
        </h1>
      </div>

      {/* Summary */}
      <div className="bg-card border border-border rounded-lg p-5 mb-6 flex gap-8 items-center">
          ...
          <div className="flex gap-0.5 justify-center mt-1">
            {[1, 2, 3, 4, 5].map((s) => (
              <Icon
                ...
                name="Star"
                size={14}
                className={s <= Math.round(Number(avgRating)) ? "fill-current" : "opacity-20"}
                style={{ color: "hsl(16 90% 55%)" }}
              />
            ))}
          </div>
        ...
        <div className="flex-1 space-y-1.5">
          {ratingCounts.map(({ stars, count }) => (
            <div key={stars} className="flex items-center gap-2">
              ...
              <Icon name="Star" size={11} style={{ color: "hsl(16 90% 55%)" }} className="fill-current shrink-0" />
              <div className="flex-1 bg-muted rounded-full h-1.5 overflow-hidden">
                <div
                  className="h-full rounded-full transition-all"
                  style={{ width: `${(count / REVIEWS.length) * 100}%`, backgroundColor: "hsl(16 90% 55%)" }}
                />
              </div>
                ...
                <div className="flex gap-0.5 mt-0.5">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <Icon
                      ...
                      name="Star"
                      size={11}
                      className={s <= r.rating ? "fill-current" : "opacity-20"}
                      style={{ color: "hsl(16 90% 55%)" }}
                    />
                  ))}
                </div>
